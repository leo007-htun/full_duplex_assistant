...
# Realtime model (env-overridable)
OPENAI_REALTIME_MODEL = os.getenv("OPENAI_REALTIME_MODEL", "gpt-realtime")
OPENAI_REALTIME_URL = f"wss://api.openai.com/v1/realtime?model={OPENAI_REALTIME_MODEL}"

# Commit-gating server side
MIN_COMMIT_MS = float(os.getenv("MIN_COMMIT_MS", "120"))           # must be >= 100ms
AUTO_COMMIT_EVERY_MS = float(os.getenv("AUTO_COMMIT_EVERY_MS", "1500"))
...
# =========================================================
# Server-mic loop (optional local dev)
# =========================================================
  ...
  async with websockets.connect(OPENAI_REALTIME_URL, extra_headers=headers) as upstream:
      ...
      async def send_audio():
          while True:
              ...
              # server VAD → no per-frame commit
...
async def ws_stream(ws: WebSocket):
  ...
  # ---- commit gating state ----
  pending_ms = 0.0             # ms appended since last commit
  flush_pending = False        # browser asked to flush but we didn't have enough yet
  last_commit_at = time.time() # for periodic auto-commit
  last_sr = 16000              # last declared frame SR (client sends 16000)

  async def send_commit_if_ready(source="manual"):
      nonlocal pending_ms, flush_pending, last_commit_at
      ...
      if pending_ms >= MIN_COMMIT_MS:
          ...
          await ws.send_text(json.dumps({"type": "commit_sent", "ms": round(pending_ms,1), "src": source}))
          pending_ms = 0.0
          flush_pending = False
  ...
  async def auto_commit_pumper():
      # Periodically commit if enough audio is sitting uncommitted
      while True:
          try:
              await asyncio.sleep(0.25)
              if not upstream_ref["conn"]:
                  continue
              since = (time.time() - last_commit_at) * 1000.0
              if pending_ms >= MIN_COMMIT_MS and since >= AUTO_COMMIT_EVERY_MS:
                  await send_commit_if_ready(source="auto")
          except Exception:
              break
  ...
  async def connect_upstream():
      ...
      try:
          ...
          await upstream.send(json.dumps({
              "type": "session.update",
              "session": {
                  "input_audio_format": "pcm16",  # NOTE: string only (no sample_rate_hz)
                  "input_audio_transcription": {"model": "whisper-1", "language": "en"},
                  "turn_detection": {"type": "server_vad", "threshold": 0.32,
                                     "prefix_padding_ms": 300, "silence_duration_ms": 350},
                  "input_audio_noise_reduction": {"type": "near_field"},
  ...
  # Start connecting & pumper (non-blocking)
  asyncio.create_task(connect_upstream())
  asyncio.create_task(auto_commit_pumper())
      ...
      try:
          while True:
              ...
              if t == "ping":
                  ...
                  # Track pending ms for commit gating
                  ...
                  # If a flush was deferred earlier, commit as soon as we have enough
