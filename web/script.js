  ...
  async function startRealtime(userMediaStream) {
    try {
      ...
      ws.onmessage = (ev) => {
        ...
        if (t === "conversation.item.input_audio_transcription.delta") { appendTranscript(msg.delta || ""); 
          return; }
        if (t === "conversation.item.input_audio_transcription.completed") { finalizeTranscript(msg.transcript || ""); 
          return; }
...
// Assistant text deltas for captions
  ...
  function maybeCommitToServer(force = false) {
    ...
    try { ws.send(JSON.stringify({ type: "input_audio_buffer.commit" })); 
    lastCommitAt = now; 
    appendedMsSinceCommit = 0; 
  } catch {}
  }
